from canvas import *
from tkinter_project.buying_page import display_products
from tkinter_project.helpers import clean_screen
...
def login():
    ...
    frame.create_window(160, 150, window=logging_btn)
    ...
    for el in list(info.values())[:-1]:
        if el.strip() == "":
            frame.create_text(
                300,
                300,
                text="We are missing some information! Please check all the fields necessary!",
                fill="red",
                tag="error"
            )
            return False
